...
export const initWeatherWidget = async () => {
    ...
    const fetchWeather = async () => {
        try {
            ...
            const descEl = document.getElementById('weather-desc-portal') || document.getElementById('weather-desc')

            // Coords for Duy Ha, Ninh Binh area: 20.67, 105.92
            const url = 'https://api.open-meteo.com/v1/forecast?latitude=20.67&longitude=105.92&current=temperature_2m,relative_humidity_2m,weather_code,wind_speed_10m&timezone=Asia%2FBangkok'
            ...
            if (data && data.current) {
                ...
                if (descEl) descEl.textContent = 'Duy Hà'
